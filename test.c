...
static void test_parse_true() {
    lept_value v;
    lept_init(&v);
    lept_set_boolean(&v, 0); /* 这样一开始的类型会被设置为LEPT_FALSE 有利于第二个测试*/
...
static void test_parse_false() {
    lept_value v;
    lept_init(&v);
    lept_set_boolean(&v, 1); /* 这样一开始的类型会被设置为LEPT_TRUE 有利于第二个测试*/
...
#define TEST_STRING(expect, json_str) \
    do { \
        lept_value v; \
        lept_init(&v); \
        EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, json_str)); \
...
#define TEST_ERROR(error, json_str) \
    do { \
        lept_value v; \
        lept_init(&v); \
        v.type = LEPT_FALSE; \
        EXPECT_EQ_INT(error, lept_parse(&v, json_str)); \
        EXPECT_EQ_INT(LEPT_NULL, lept_get_type(&v)); \
        lept_free(&v); \
    } while(0)
/* 异常值测试*/
...
static void test_access_number() {
    ...
    lept_init(&v);
    lept_set_string(&v, "a", 1);
    lept_set_number(&v, 1.25);
    EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(&v));
    EXPECT_EQ_INT(1.25, lept_get_number(&v));
    lept_free(&v);
}
